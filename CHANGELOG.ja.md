...
**パイプラインのキューがデータグリッドに** — ソート可能な列、⋯ 行メニュー、キーボードショートカット。

### 追加
- **キューをデータグリッドとして表示** — `data/pipeline.md` の行（`url | 会社 | 職種 | …`）から解析した `#`、会社、職種、勤務地、報酬、日付、メモ、URL の列。自由形式の列は形（日付 / 報酬 / 勤務地 / メモ）で分類されるためファイルの情報が黙って落ちることはなく、メタデータのない URL はホストと整形したスラッグで補完します。固定ヘッダーのクリックソート、既定は新しい求人が先頭。1000 行超の仮想化は従来どおりです。
- **行アクションを ⋯ メニューに集約** — 評価 / 開く / 完了 / スキップ / 削除を `body` 直下の単一ポップオーバーにまとめました（行内メニューはスクロールするカードで切れるため）。画面端では上側に開き、行ごとの `aria-label` は維持されます。
- **行メニューのキーボードショートカット** — `E` 評価、`O` 開く、`D` 完了、`S` スキップ、`X` 削除、`Esc` で閉じる。キーは固定（翻訳された頭文字ではない）で各項目にバッジ表示されるため、17 言語すべてで同じように動きます。
