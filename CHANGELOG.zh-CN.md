...
**流水线队列变成数据网格** — 可排序列、一个 ⋯ 行菜单和键盘快捷键。

### 新增
- **队列以数据网格呈现** — 从 `data/pipeline.md` 的行（`url | 公司 | 职位 | …`）解析出 `#`、公司、职位、地点、薪酬、日期、备注、URL 列。自由格式的列按形态分类（日期 / 薪酬 / 地点 / 备注），文件里的内容不会被悄悄丢弃；没有元数据的 URL 回退到主机名加美化后的 slug。粘性表头点击排序，默认最新职位在前。超过 1000 行的虚拟化保持不变。
- **行操作收进 ⋯ 菜单** — 评估 / 打开 / 完成 / 跳过 / 删除现在集中在一个 `body` 级弹出层里（行内菜单会被可滚动卡片裁切）；靠近视口边缘时向上弹出，并保留每行的 `aria-label`。
- **行菜单的键盘快捷键** — `E` 评估、`O` 打开、`D` 完成、`S` 跳过、`X` 删除，`Esc` 关闭。按键是固定的（不是翻译后的首字母），并以徽章显示在每一项上，因此在 17 种语言下表现一致。
