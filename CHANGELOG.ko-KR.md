...
**파이프라인 대기열이 데이터 그리드로** — 정렬 가능한 열, ⋯ 행 메뉴, 키보드 단축키.

### 추가됨
- **대기열이 데이터 그리드로 표시됩니다** — `data/pipeline.md` 행(`url | 회사 | 직무 | …`)에서 파싱한 `#`, 회사, 직무, 지역, 보수, 날짜, 메모, URL 열. 자유 형식 열은 형태(날짜 / 보수 / 지역 / 메모)로 분류되어 파일의 내용이 조용히 사라지지 않으며, 메타데이터 없는 URL은 호스트와 정리된 슬러그로 대체됩니다. 고정 헤더 클릭 정렬, 기본값은 최신 공고 우선. 1000행 이상의 가상화는 그대로입니다.
- **행 동작을 ⋯ 메뉴로 통합** — 평가 / 열기 / 완료 / 건너뛰기 / 삭제가 `body` 레벨 팝오버 하나로 모였습니다(행 안의 메뉴는 스크롤되는 카드에 잘림). 화면 가장자리에서는 위쪽으로 열리며 행별 `aria-label`은 유지됩니다.
- **행 메뉴의 키보드 단축키** — `E` 평가, `O` 열기, `D` 완료, `S` 건너뛰기, `X` 삭제, `Esc` 닫기. 키는 고정이며(번역된 첫 글자가 아님) 각 항목에 배지로 표시되어 17개 언어에서 동일하게 작동합니다.
