...
**流水線佇列變成資料網格** — 可排序欄位、一個 ⋯ 列選單與鍵盤快速鍵。

### 新增
- **佇列以資料網格呈現** — 從 `data/pipeline.md` 的列（`url | 公司 | 職位 | …`）解析出 `#`、公司、職位、地點、薪酬、日期、備註、URL 欄位。自由格式欄位依形態分類（日期 / 薪酬 / 地點 / 備註），檔案中的內容不會被靜默丟棄；沒有中繼資料的 URL 會退回主機名稱加上美化的 slug。黏性表頭點擊排序，預設最新職缺在前。超過 1000 列的虛擬化維持不變。
- **列動作收進 ⋯ 選單** — 評估 / 開啟 / 完成 / 略過 / 刪除現在集中在單一 `body` 層級的浮動選單（列內選單會被可捲動卡片裁切）；靠近視窗邊緣時向上開啟，並保留每列的 `aria-label`。
- **列選單的鍵盤快速鍵** — `E` 評估、`O` 開啟、`D` 完成、`S` 略過、`X` 刪除，`Esc` 關閉。按鍵固定（不是翻譯後的首字母），並以徽章顯示於每個項目，因此在 17 種語言下表現一致。
