...
**Очередь пайплайна стала таблицей данных** — сортируемые колонки, одно меню строки ⋯ и горячие клавиши.

### Добавлено
- **Очередь отображается как таблица данных** — колонки `#`, Компания, Роль, Локация, Зарплата, Дата, Заметки и URL, разобранные из строк `data/pipeline.md` (`url | Компания | Роль | …`). Свободные колонки классифицируются по форме (дата / зарплата / локация / заметка), поэтому ничего из файла не теряется молча; для голых URL подставляются хост и читаемый слаг. Липкая шапка с сортировкой по клику, по умолчанию — самые свежие вакансии сверху. Виртуализация свыше 1000 строк не изменилась.
- **Действия строки собраны в меню ⋯** — Оценить / Открыть / Готово / Пропустить / Удалить теперь живут в одном поповере на уровне `body` (меню внутри строки обрезалось бы прокручиваемой карточкой); у края экрана оно открывается вверх, `aria-label` для каждой строки сохранены.
- **Горячие клавиши в меню строки** — `E` оценить, `O` открыть, `D` готово, `S` пропустить, `X` удалить, `Esc` закрывает. Клавиши фиксированные (не первая буква перевода) и показаны бейджами у каждого пункта, поэтому работают одинаково во всех 17 языках.
